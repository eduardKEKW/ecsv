...
class ProductFlat extends Model implements ProductFlatContract
{
    ...
    {
        return $this->attributes["reviews_average"];
    }
    ...
    {
        if(is_null($this->attributes["url_key"])) 
        {
            return Str::slug($this->attributes["name"], '-') . '/?p=' . $this->attributes["product_id"];
        } else 
        {
            return $this->attributes["url_key"];
        }
    }
