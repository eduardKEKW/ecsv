...
class WishlistMutation extends Controller
{
    ...
    {
        $params = isset($args['input']) ? $args['input'] : (isset($args['customer_id']) ? $args : []);
// throw new \Exception(json_encode($args), 1);
        ...
        try {
            ...
            $wishlists = app(WishlistRepository::class)->scopeQuery(function ($query) use ($params) {
                ...
                if (isset($params['ip']) && $params['ip']) {
                    $qb->where('wishlist.is_guest', $params['is_guest']);
                    $qb->where('wishlist.ip', $params['ip']);
                }
